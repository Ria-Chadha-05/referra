...
async def lifespan(app: FastAPI):
    ...
    # Single shared, connection-pooled httpx.AsyncClient for all outbound
    # calls to Semantic Scholar / OpenAlex. Created once here and reused for
    # the lifetime of the app so we don't pay TLS/connection setup cost (or
    # look "bursty" to upstream rate limiters) on every single request.
    await init_http_client()
    logger.info("Shared outbound HTTP client ready.")

    yield

    # Clean shutdown — release pooled connections.
